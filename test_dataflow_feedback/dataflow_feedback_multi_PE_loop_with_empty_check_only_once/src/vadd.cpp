...
void PE_A(
	...
	for (int i = 0; i < LOOP_COUNT; i++) {
		...
		// End by 
		if (first_iter) {
			while (s_D_to_A.empty()) {}
			first_iter = false;
		} 
		int out = s_D_to_A.read();
		out_intersect[i] = out;
	}
}
...
void PE_D(
	...
	for (int i = 0; i < LOOP_COUNT; i++) {
		if (first_iter) {
			while (s_C_to_D.empty()) {}
			first_iter = false;
		}
		int in = s_C_to_D.read();
...
#pragma HLS stream variable=s_B_to_C depth=512
    hls::stream<int> s_C_to_D("C to D"); 
#pragma HLS stream variable=s_C_to_D depth=512
    hls::stream<int> s_D_to_A("D to A"); 
#pragma HLS stream variable=s_D_to_A depth=512
