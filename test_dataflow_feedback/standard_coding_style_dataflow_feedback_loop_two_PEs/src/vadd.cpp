...
// Coding style for the controller of the feedback loop (which send data before receiveing results),
//   use a first iteration flag to run empty check for the first iteration
void PE_A(
    // in
...
// Coding style for the rest of the PEs (which receive results before sending data),
//   check for necessary input data before proceeding to the loop
void PE_B(
    // in
    int* layer_cache,
    hls::stream<int>& s_A_to_B,
