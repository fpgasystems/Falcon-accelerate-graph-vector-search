...
#pragma HLS array_partition variable=queue_replication_array complete
	...
	for (int qid = 0; qid < query_num; qid++) {
		...
		int num_insertion_per_iter = 0; // number of insertions of a sync iteration from all channels
		...
		while (!stop) {
			if (!s_num_inserted_candidates.empty()) { // each channel send a number, regardless of per-iter batch size
				...
				num_insertion_per_iter += num_insertion;
				...
				if (recv_channels_left == 0) {
					...
					// only trigger new task dispatching if there is either 
					//   1. new candidates inserted in this iteration
					//   2. this is the last on the fly stage already
					if (num_insertion_per_iter > 0 || on_the_fly_async_stage_num == 0) {
					...
					} else { // consume largest_result_queue_elements
						wait_data_fifo_first_iter<float>(
							1, s_largest_result_queue_elements, first_iter_s_largest_result_queue_elements);
						float threshold = s_largest_result_queue_elements.read();
					}
					...
					// reset num_insertion_per_iter
					num_insertion_per_iter = 0;
