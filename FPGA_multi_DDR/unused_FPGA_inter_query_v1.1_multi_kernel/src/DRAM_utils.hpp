...
void split_queries(
	...
	// out streams
	hls::stream<int>& s_query_batch_size_chan_0,
#if N_CHANNEL >= 2
	hls::stream<int>& s_query_batch_size_chan_1,
#endif
#if N_CHANNEL >= 3
	hls::stream<int>& s_query_batch_size_chan_2,
#endif
#if N_CHANNEL >= 4
	hls::stream<int>& s_query_batch_size_chan_3,
#endif

	hls::stream<ap_uint<512>>& s_query_vectors_in_chan_0,
#if N_CHANNEL >= 2
	hls::stream<ap_uint<512>>& s_query_vectors_in_chan_1,
#endif
#if N_CHANNEL >= 3
	hls::stream<ap_uint<512>>& s_query_vectors_in_chan_2,
#endif
#if N_CHANNEL >= 4
	hls::stream<ap_uint<512>>& s_query_vectors_in_chan_3,
#endif

	hls::stream<int>& s_entry_point_ids_chan_0
#if N_CHANNEL >= 2
	, hls::stream<int>& s_entry_point_ids_chan_1
#endif
#if N_CHANNEL >= 3
	, hls::stream<int>& s_entry_point_ids_chan_2
#endif
#if N_CHANNEL >= 4
	, hls::stream<int>& s_entry_point_ids_chan_3
#endif
) {
	...
	while (true) {
		...
		if (query_num == -1) {
			s_query_batch_size_chan_0.write(-1);
#if N_CHANNEL >= 2
			s_query_batch_size_chan_1.write(-1);
#endif
#if N_CHANNEL >= 3
			s_query_batch_size_chan_2.write(-1);
#endif
#if N_CHANNEL >= 4
			s_query_batch_size_chan_3.write(-1);
#endif
			break;
		}
		...
		int	query_batch_size_chan_0 = 0 < remainder_batch_size_per_channel? 
				base_batch_size_per_channel + 1 : base_batch_size_per_channel;
		s_query_batch_size_chan_0.write(query_batch_size_chan_0);
#if N_CHANNEL >= 2
		int query_batch_size_chan_1 = 1 < remainder_batch_size_per_channel? 
				base_batch_size_per_channel + 1 : base_batch_size_per_channel;
		s_query_batch_size_chan_1.write(query_batch_size_chan_1);
#endif
#if N_CHANNEL >= 3
		int query_batch_size_chan_2 = 2 < remainder_batch_size_per_channel? 
				base_batch_size_per_channel + 1 : base_batch_size_per_channel;
		s_query_batch_size_chan_2.write(query_batch_size_chan_2);
#endif
#if N_CHANNEL >= 4
		int query_batch_size_chan_3 = 3 < remainder_batch_size_per_channel? 
				base_batch_size_per_channel + 1 : base_batch_size_per_channel;
		s_query_batch_size_chan_3.write(query_batch_size_chan_3);
#endif
			...
			switch (this_channel) {
				case 0:
					...
					#pragma HLS pipeline II=1
						...
						s_query_vectors_in_chan_0.write(query_vector_AXI);
					}
					s_entry_point_ids_chan_0.write(s_entry_point_ids.read());
					break;
#if N_CHANNEL >= 2
				case 1:
					for (int j = 0; j < vec_AXI_num; j++) {
					#pragma HLS pipeline II=1
						ap_uint<512> query_vector_AXI = s_query_vectors_in.read();
						s_query_vectors_in_chan_1.write(query_vector_AXI);
					}
					s_entry_point_ids_chan_1.write(s_entry_point_ids.read());
					break;
#endif
#if N_CHANNEL >= 3
				case 2:
					...
					#pragma HLS pipeline II=1
						ap_uint<512> query_vector_AXI = s_query_vectors_in.read();
						s_query_vectors_in_chan_2.write(query_vector_AXI);
					}
					s_entry_point_ids_chan_2.write(s_entry_point_ids.read());
					break;
#endif
#if N_CHANNEL >= 4
				case 3:
					...
					#pragma HLS pipeline II=1
						ap_uint<512> query_vector_AXI = s_query_vectors_in.read();
						s_query_vectors_in_chan_3.write(query_vector_AXI);
					}
					s_entry_point_ids_chan_3.write(s_entry_point_ids.read());
					break;
#endif
			}
...
void write_results(
	...
	hls::stream<int>& s_out_ids_chan_0,
#if N_CHANNEL >= 2
	hls::stream<int>& s_out_ids_chan_1,
#endif
#if N_CHANNEL >= 3	
	hls::stream<int>& s_out_ids_chan_2,
#endif
#if N_CHANNEL >= 4
	hls::stream<int>& s_out_ids_chan_3,
#endif

	hls::stream<float>& s_out_dists_chan_0,
#if N_CHANNEL >= 2
	hls::stream<float>& s_out_dists_chan_1,
#endif
#if N_CHANNEL >= 3
	hls::stream<float>& s_out_dists_chan_2,
#endif
#if N_CHANNEL >= 4
	hls::stream<float>& s_out_dists_chan_3,
#endif

	hls::stream<int>& s_debug_signals_chan_0,
#if N_CHANNEL >= 2
	hls::stream<int>& s_debug_signals_chan_1,
#endif
#if N_CHANNEL >= 3
	hls::stream<int>& s_debug_signals_chan_2,
#endif
#if N_CHANNEL >= 4
	hls::stream<int>& s_debug_signals_chan_3,
#endif
	...
	while (true) {
		...
		while (remained_query_num > 0) {
			...
			switch (this_channel) {
				case 0:
					wait_data_fifo_first_iter<int>(
						ef, s_out_ids_chan_0, first_iter_s_out_ids_per_channel[this_channel]);
					wait_data_fifo_first_iter<float>(
						ef, s_out_dists_chan_0, first_iter_s_out_dists_per_channel[this_channel]);
					wait_data_fifo_first_iter<int>(
						debug_size, s_debug_signals_chan_0, first_iter_s_debug_signals_per_channel[this_channel]);
					
					// use two loops to infer burst per loop
					for (int i = 0; i < ef; i++) {
					#pragma HLS pipeline II=1
						int start_addr = processed_query_num * ef + i;
						out_id[start_addr] = s_out_ids_chan_0.read();
					}
					for (int i = 0; i < ef; i++) {
					#pragma HLS pipeline II=1
						int start_addr = processed_query_num * ef + i;
						out_dist[start_addr] = s_out_dists_chan_0.read();
					}
					for (int i = 0; i < debug_size; i++) {
					#pragma HLS pipeline II=1
						int start_addr = processed_query_num * debug_size + i;
						mem_debug[start_addr] = s_debug_signals_chan_0.read();
					}
					break;
#if N_CHANNEL >= 2
				case 1:
					wait_data_fifo_first_iter<int>(
						ef, s_out_ids_chan_1, first_iter_s_out_ids_per_channel[this_channel]);
					wait_data_fifo_first_iter<float>(
						ef, s_out_dists_chan_1, first_iter_s_out_dists_per_channel[this_channel]);
					wait_data_fifo_first_iter<int>(
						debug_size, s_debug_signals_chan_1, first_iter_s_debug_signals_per_channel[this_channel]);
					
					// use two loops to infer burst per loop
					for (int i = 0; i < ef; i++) {
					#pragma HLS pipeline II=1
						int start_addr = processed_query_num * ef + i;
						out_id[start_addr] = s_out_ids_chan_1.read();
					}
					for (int i = 0; i < ef; i++) {
					#pragma HLS pipeline II=1
						int start_addr = processed_query_num * ef + i;
						out_dist[start_addr] = s_out_dists_chan_1.read();
					}
					for (int i = 0; i < debug_size; i++) {
					#pragma HLS pipeline II=1
						int start_addr = processed_query_num * debug_size + i;
						mem_debug[start_addr] = s_debug_signals_chan_1.read();
					}
					break;
#endif
#if N_CHANNEL >= 3
				case 2:
					wait_data_fifo_first_iter<int>(
						ef, s_out_ids_chan_2, first_iter_s_out_ids_per_channel[this_channel]);
					wait_data_fifo_first_iter<float>(
						ef, s_out_dists_chan_2, first_iter_s_out_dists_per_channel[this_channel]);
					wait_data_fifo_first_iter<int>(
						debug_size, s_debug_signals_chan_2, first_iter_s_debug_signals_per_channel[this_channel]);
					
					// use two loops to infer burst per loop
					for (int i = 0; i < ef; i++) {
					#pragma HLS pipeline II=1
						int start_addr = processed_query_num * ef + i;
						out_id[start_addr] = s_out_ids_chan_2.read();
					}
					for (int i = 0; i < ef; i++) {
					#pragma HLS pipeline II=1
						int start_addr = processed_query_num * ef + i;
						out_dist[start_addr] = s_out_dists_chan_2.read();
					}
					for (int i = 0; i < debug_size; i++) {
					#pragma HLS pipeline II=1
						int start_addr = processed_query_num * debug_size + i;
						mem_debug[start_addr] = s_debug_signals_chan_2.read();
					}
					break;
#endif
#if N_CHANNEL >= 4
				case 3:
					wait_data_fifo_first_iter<int>(
						ef, s_out_ids_chan_3, first_iter_s_out_ids_per_channel[this_channel]);
					wait_data_fifo_first_iter<float>(
						ef, s_out_dists_chan_3, first_iter_s_out_dists_per_channel[this_channel]);
					wait_data_fifo_first_iter<int>(
						debug_size, s_debug_signals_chan_3, first_iter_s_debug_signals_per_channel[this_channel]);
					
					// use two loops to infer burst per loop
					for (int i = 0; i < ef; i++) {
					#pragma HLS pipeline II=1
						int start_addr = processed_query_num * ef + i;
						out_id[start_addr] = s_out_ids_chan_3.read();
					}
					for (int i = 0; i < ef; i++) {
					#pragma HLS pipeline II=1
						int start_addr = processed_query_num * ef + i;
						out_dist[start_addr] = s_out_dists_chan_3.read();
					}
					for (int i = 0; i < debug_size; i++) {
					#pragma HLS pipeline II=1
						int start_addr = processed_query_num * debug_size + i;
						mem_debug[start_addr] = s_debug_signals_chan_3.read();
					}
					break;
#endif					
			}
