...
template<const int replication_factor>
void replicate_s_fetch_batch_size(
    // in initialization
    const int query_num,
    ...
    for (int qid = 0; qid < query_num; qid++) {
        while (true) {
            if (!s_finish_query_in.empty() && s_fetch_batch_size.empty()) {
                ...
                #pragma HLS unroll
                    s_fetch_batch_size_replicated[i].write(fetch_batch_size);
                }
            }
        }
    }
    ...
    for (int qid = 0; qid < query_num; qid++) {
        ...
        while (true) {
            ...
            if (!s_finish_query_in.empty() && s_fetch_batch_size_replicated.empty() && s_fetched_vectors.empty()) {
                s_finish_query_out.write(s_finish_query_in.read());
                break;
            } else if (!s_fetch_batch_size_replicated.empty() && !s_fetched_vectors.empty()) {
    ...
    for (int qid = 0; qid < query_num; qid++) {
        ...
        while (true) {
            ...
            if (!s_finish_query_in.empty() && s_fetch_batch_size_replicated.empty() && s_partial_distances.empty()) {
                s_finish_query_out.write(s_finish_query_in.read());
                break;
            } else if (!s_fetch_batch_size_replicated.empty() && !s_partial_distances.empty()) {
    ...
    for (int qid = 0; qid < query_num; qid++) {
        ...
        while (true) {
            ...
            if (!s_finish_query_in.empty() && s_fetch_batch_size_replicated.empty() && s_partial_distances_packed.empty() 
                ...
                s_finish_query_out.write(s_finish_query_in.read());
                break;
            } else if (!s_fetch_batch_size_replicated.empty() && !s_partial_distances_packed.empty() && !s_fetch_valid.empty() &&!s_fetched_neighbor_ids_replicated.empty()) {
...
#pragma HLS stream variable=s_partial_distances_packed depth=16
    ...
    replicate_s_fetch_batch_size<replication_factor>(
        // in initialization
        query_num,
        s_fetch_batch_size,
        s_finish_query_in,
