...
void read_memory(
	...
	for (int i = 0; i < query_num; i++) {
		...
		for (int i = 0; i < num_fetched_vectors_per_query; i += read_batch_size) {
		#pragma HLS pipeline II=1
			int remaining = num_fetched_vectors_per_query - i;
			int num_elements = (remaining < read_batch_size) ? remaining : read_batch_size;
			s_fetch_batch_size.write(num_elements);
			// write fetched vector & s_fetched_neighbor_ids_replicated 
